...
public class ProducingOrderDialog extends Dialog<OrderVO>{
		...
		Callback<ButtonType, OrderVO> resultConverter = new Callback<ButtonType	, OrderVO>() {
			...
			public OrderVO call(ButtonType param) {
				...
				if(param.getButtonData() == ButtonData.OK_DONE){
					return new OrderVO(-1, null, customerID, null, null, hotelVO.hotelName, roomTypeChoiceBox.getValue(), Integer.valueOf(numTextField.getText()), 
							childrenCheckBox.isSelected(), checkInDatePicker.getValue(), checkOutDatePicker.getValue(), Integer.valueOf(totalText.getText()), "正常");
				}
				else {
					return null;
				}
	...
	private void initUI(int roomIndex){
		...
		Callback<DatePicker, DateCell> checkInDayCellFactory = 
	    ...
	    Callback<DatePicker, DateCell> checkOutDayCellFactory = 
	            new Callback<DatePicker, DateCell>() {
	                @Override
	                public DateCell call(final DatePicker datePicker) {
	                    return new DateCell() {
